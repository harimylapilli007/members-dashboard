...
export default function Component() {
  ...
  const MembershipModal = ({ membership, onClose }: { membership: any, onClose: () => void }) => {
    ...
    const handleDragStop = (e: any, data: any) => {
      if (Math.abs(data.y) > dragThreshold) {
        onClose()
      } else {
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40 p-4">
        <Draggable
          ...
          bounds={{ top: 0, bottom: 1000 }}
          nodeRef={modalRef as React.RefObject<HTMLElement>}
        >
          <div 
            ref={modalRef}
            className={`relative bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto transition-transform duration-200 ${
          ...
          >
            ...
            <div className="bg-[#a07735] p-3 sm:p-4 rounded-t-2xl text-center relative">
              <div className="flex justify-center">
                <div className="w-12 h-1.5 bg-white/30 rounded-full animate-pulse" />
              </div>
